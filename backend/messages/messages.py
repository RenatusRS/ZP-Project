...
def read_message(user: str, message: bytes) -> str:
	...
	return message[Cfg.TIMESTAMP_BYTE_SIZE:].decode('utf8'), (int.from_bytes(public_key_id, sys.byteorder), valid) if f_auth else None, (SymEnc(f_sym), int.from_bytes(private_key_ring.key_id, sys.byteorder)) if f_sym and f_asym else None, f_comp, f_radix
